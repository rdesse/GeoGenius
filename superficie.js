...
function updateScore(score) {
    var meilleurScore = localStorage.getItem('highscoreSup');

    if (score > meilleurScore) {
        localStorage.setItem('highscoreSup', score);
    }
}
...
function meilleurScore() {
    ...
    if (meilleurScore !== null) {
        ret=localStorage.getItem('highscoreSup');
    } else {
        ret=0;
    }
...
function avanceTour() {
    if(reussi==true && score != tourMax){
        ...
        $("#supPaysA").html(paysA.superficie.toLocaleString('fr-FR', options)+"km²");
