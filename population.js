...
function updateScore(score) {
    var meilleurScore = localStorage.getItem('highscorePop');

    if (score > meilleurScore) {
        localStorage.setItem('highscorePop', score);
    }
}
...
function meilleurScore() {
    ...
    if (meilleurScore !== null) {
        ret=localStorage.getItem('highscorePop');
    } else {
        ret=0;
    }
...
function avanceTour() {
    if(reussi==true && score != tourMax){
        ...
        $("#popPaysA").html(paysA.population.toLocaleString('fr-FR', options)+" millions d'habitants");
